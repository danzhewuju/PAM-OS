...
这份文档说明如何让大模型通过 PAM-OS skill 使用本地长期记忆。当前推荐路径是 MCP tools 优先；只有 fallback 到 REST 或 CLI 时，才需要读取 skill 自带的配置文件。
...
只有 fallback 到 CLI 或 REST 时，大模型才需要读取 skill 目录里的 `config.toml`。安装器默认写入这些位置：
...
REST 模式适合 MCP 不可用、但你希望大模型直接通过 HTTP API 使用 PAM-OS 的场景。
...
推荐使用插件安装器注册 MCP server 和全局 skill：

```bash
./scripts/install-plugin.sh --codex --yes
```

重启 Codex 后，优先使用 `pam_os_memory` MCP server 暴露的工具。skill 负责判断什么时候读写记忆。
...
如果 MCP 不可用，并且要让大模型走 REST fallback：

1. 修改 `~/.codex/skills/pam-os-memory/config.toml`，设置 `mode = "rest"`。
2. 启动 REST server。
...
安装后重启对应客户端。MCP 可用时优先走 MCP；否则按 guidance 和 skill config 使用 REST/CLI fallback。
...
如果 MCP 可用，大模型应该优先使用 MCP tool。

如果 fallback 到 `mode = "cli"`，大模型应该使用本地 CLI 命令。
