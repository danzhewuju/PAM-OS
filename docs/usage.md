...
- Claude Code：`~/.claude/skills/pam-os-memory`，并通过 `claude mcp add-json --scope user` 注册 `pam_os_memory` MCP server
...
| `observe_turn` | 观察完整对话轮次，执行保守的自动记忆/策略学习并写入审计。 |
...
用户提问
  ...
  -> 模型回答
  -> 调用 observe_turn 观察完整轮次，自动处理低风险捕获和 policy signal 准入
...
**观察轮次**（回答后）：调用 `observe_turn` 观察用户消息和模型回答。它会尝试自动捕获低风险稳定记忆，并通过本地准入控制器为明确长期指令或短跟进表达生成 `policy_signals` 候选/active 信号，同时写入 `quality_traces`。

**写记忆**（回答后）：当用户透露稳定信息时才写入（长期偏好、项目决策、风格要求、用户纠正等），跳过"哈哈好的"这类临时闲聊。`observe_turn` 会自动走一次保守捕获；非常明确或导入类内容仍可直接调用 `capture_memory`。
