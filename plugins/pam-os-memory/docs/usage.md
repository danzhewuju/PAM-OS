...
The skill config contains observable skill, API, and detected server versions together with the PAM-OS REST URL, optional Basic Auth credentials, and request timeout. Re-running either platform installer detects and updates existing integrations. Existing REST settings are reused by default; interactive prompts show the previous URL and username but only report whether a password is configured. Explicit options and `PAM_OS_REST_*` environment variables take precedence. For local development, pass `--repo-dir /path/to/PAM-OS` explicitly.
