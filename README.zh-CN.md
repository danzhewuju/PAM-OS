...
PAM-OS 为助手提供一个可持久化的记忆层，可在任务前后调用。它会存储原始事件、提取结构化记忆、检索相关上下文、合并稳定的用户画像特征、学习何时应该使用记忆，并通过 CLI 或 REST 返回可直接放入提示词的上下文包。
...
- **Agent 友好**：可通过打包插件和 skill runtime 在 Codex 中使用。
...
- **协议无关核心**：同一个运行时同时支撑 CLI 和 REST API。
...
适用于 Codex、Claude Code、OpenCode 或 Hermes 集成。安装器会在 `~/.local/share/pam-os/repo` 维护一个托管的 PAM-OS checkout，安装所选客户端集成，并写入选择 CLI 或 REST 的 skill `config.toml`。
...
默认规则实现位于 `pam_os.rule_provider`。未来的 LLM 或 embedding provider 可以接入同一套接口，而无需修改 CLI、REST 或公开运行时方法。
...
skill 每次操作前读取 `config.toml`。`mode = "cli"` 运行本地 `memory` 命令；`mode = "rest"` 调用配置的 REST API。
